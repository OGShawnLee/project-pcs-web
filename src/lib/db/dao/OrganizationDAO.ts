...
export default class OrganizationDAO {
  ...
  public static async getMany(limit = 12) {
    const { data, error } = await DBClient.from("organization").select("*").limit(limit).order("created_at", { ascending: false });
    return createResult(
      data ? data.map((organization) => Schema.getValidOrganization({
